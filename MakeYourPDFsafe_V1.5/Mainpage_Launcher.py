...
def Mainpage():
    ...
    folderopenfilter = dict(title="Select a folter to contain your protected file.")
    ...
    shbuttxt = tk.StringVar()
    shbuttxt.set("Show Password")
    
    Protected_File_Image = PhotoImage(file=main_path+"\\imgs\\img2.png")
    Label(mpage, image=Protected_File_Image, border=0).place(x=35, y=40)
    
    head_frame = Frame(mpage, width=450, height=480, background="white")
    head_frame.place(x=464, y=10)
    
    head_title = Label(head_frame, width=30, background="white", text="Make Your PDF Safe", fg="#007dfe",font=("Microsoft Himalaya light", "15",))
    ...
    def Browse_Dir():
        enpdffiledex = filedialog.askdirectory(**folderopenfilter)
    ...
    def Crypt_The_Pdf():
        global basePdf_Path, protect_pdf_destination, password
    
        basePdf_Path = pdf_file_path_label.get()
        protect_pdf_destination = en_pdf_file_path_label.get()
        password = en_pdf_file_password.get()
        repass = retype_passwd.get()

        if basePdf_Path != "Insert the path of your PDF file" and basePdf_Path != "" and basePdf_Path != " ":
            if protect_pdf_destination != "Enter the destination of the secured PDF" and protect_pdf_destination != "" and protect_pdf_destination != " ":
                if password != "Enter the password you use to unlock the PDF here" and password != "" and password != " ":
                    if password == repass:
                        basePdf_Path = basePdf_Path.replace("/", "\\")
                        protect_pdf_destination = protect_pdf_destination.replace("/", "\\")
                    
                        PdfLock.protect(basePdf_Path, password, protect_pdf_destination)
    ...
    pdf_file_path_browse_button = Button(head_frame, background="white", text="Browse Files", border=0, fg="#007dfe", cursor="hand2", command=Browse_PDF)
    ...
    en_pdf_file_path_browse_button = Button(head_frame, background="white", text="Browse Directories", border=0, fg="#007dfe", cursor="hand2", command=Browse_Dir)
    ...
    show_password_button = Button(head_frame, background="white", textvariable=shbuttxt, border=0, fg="#007dfe",cursor="hand2", command=show_password)
    ...
    Crypt_The_PDF_Button = Button(head_frame, text="Protect your PDF file", border=0, width=50, height=2, cursor="hand2", bg="#007dfe", fg="white",font=("Microsoft Himalaya light", "8", "bold"), command=Crypt_The_Pdf)
